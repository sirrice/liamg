...
def TestImapAuthentication(imap_hostname, user, xoauth_string):
  ...
  #for i in imap_conn.list()[1]: print i.split("\"/\"")
  imap_conn.select('[Gmail]/Sent Mail')
#  print imap_conn.select('INBOX')
  #print imap_conn.select('[Gmail]/All Mail')

  #typ, dat = imap_conn.search(None, '(ALL)')#BEFORE 20-Apr-2010 SINCE 1-Apr-2010)')#(SUBJECT "atwoods")'
  typ, dat = imap_conn.search(None, '(SINCE 28-Jun-2011)')#(SUBJECT "atwoods")'
#  print typ, max(map(int, dat[0].split()))
#  return
#  print typ, dat
  iternum = 0
  chunk = 5.0
  ...
  for idx in xrange(0, int(math.ceil(float(len(mids))/chunk))):
    ...
    print curids
    typ, dat = imap_conn.fetch(','.join(map(str,curids)), '(RFC822)')
    #typ, dat = imap_conn.fetch(','.join(map(str,curids)), '(RFC822.HEADER)')
    d = dat[0]
    print d[0]
    e = email.message_from_string(d[1])
    for x in e.__dict__.items():
      print x
    print '--------'
    for x in e.__dict__['_headers']:
      print x
    print '---------'
    for m in e.__dict__['_payload']:
      print '\tFOO\t',m['Content-Type']
      ctype = m['Content-Type']
      if 'multipart/alternative' in ctype :
        for x in m.__dict__['_payload']:
          print "'%s'" % x.__dict__['_payload']
      #print str(m)[100:200]
    #print dat
    exit()
    for d in dat:
...
def proc_header(header):
	
#	  print typ, dat
#	  print dat[0][1]
#  header = dat[0][1]
  e = email.message_from_string(header)
  print e
  return
  print e['message-id'],'\t',e['Subject']
  #print e.get_payload()[0]
#	  if iternum > 10: break
#	  if mid < 4100: break
